...
class OauthLogic extends Presenter
{
     ...
     * @return mixed
     */
    public function getJsCode2SessionUrl($jsCode)
    {
        $url = config('wechat.wxpay_mini_program_config.code2session_url');
        $url = str_replace(
            ['APPID_STR', 'SECRET_STR', 'JSCODE_STR'],
            [config('wechat.wxpay_mini_program_config.appid'), config('wechat.wxpay_mini_program_config.secret'), $jsCode],
            $url
        );
        return $url;
    }
