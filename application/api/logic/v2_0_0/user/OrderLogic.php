...
use app\api\model\v2_0_0\{
    FreeRuleModel, OrderModel, ShopModel, UserModel, UserHasShopModel
};
...
class OrderLogic extends Presenter
{
    /**
     * 订单支付完成后的相关操作
     *
    ...
    public function handleAfterPaymentNotify($transactionId, $outTradeNo, $receiptAmount)
    {
        try {
            ...
            // 查询店铺信息
            $shopInfo = $shopModel->getShopInfo(['shopId' => $shopId]);
            // 查询用户在店铺的免单规则
            $freeRule = $freeRuleModel->getFreeRule($freeRuleCondition);
            if (empty($freeRule)) {
                if (!empty($shopInfo['freeOrderFrequency'])) {
                    $freeRuleData = [
                        'user_id' => $userId,
                        'shop_id' => $shopId,
                        'shop_free_order_frequency' => $shopInfo['freeOrderFrequency'],
                        'order_count' => 1,
                    ...
                    if (empty($freeRuleResult['id'])) {
                        $logContent = "新增免单规则失败，用户id={$userId}, 订单编号={$outTradeNo}, 免单次数={$shopInfo['freeOrderFrequency']}";
    ...
    public function pushMessageAfterPayment($order, $user, $shop, $nowTime)
    {
        ...
        UserMessageModel::create([
            ...
            'title' => ($freeOrderFlag == 1) ? 0 : $order['payment_amount'],
            'content' => ($freeOrderFlag == 1) ? 0 : $order['payment_amount'],
            'read_status' => 0,
            'delete_status' => 0,
            'generate_time' => $nowTime
        ]);
