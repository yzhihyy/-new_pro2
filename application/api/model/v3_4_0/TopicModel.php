    ...
    public function getTopicList(array $where)
    {
        $query = $this->alias('t')
            ->field([
                ...
                't.topic_width AS topicWidth',
                't.topic_height AS topicHeight',
            ])
            ->join('topic_video_relation tvr', 'tvr.topic_id = t.id')
            ->join('video v', 'v.id = tvr.video_id AND v.status = 1 AND v.visible = 1 AND v.audit_status = 1')
        ...
        if (isset($where['cityId']) && !empty($where['cityId'])) {
            $query->where('t.city_id', $where['cityId']);
        }

        // 获取指定标签下的视频或随记
        if (isset($where['tagIds']) && !empty($where['tagIds'])) {
            $query->join('video_tag_relation vtr', "vtr.video_id = v.id AND vtr.tag_id IN ({$where['tagIds']})");
        }

        return $query->order('t.is_recommend', 'desc')
                ->order('t.id', 'desc')
